...
public class VernacularConfig
{
    ...
    private Supplier<Boolean> useLocalMousePointer = () -> true;
    ...
    /**
     * Specifies a Supplier which will be called to find the VNC username if the remote host uses an authentication scheme that requires one.
     *
    ...
    /**
     * Specifies a Consumer which will be passed an Image representing the remote server's mouse pointer image, and a Point representing its hotspot (i.e. the
     * point within the cursor that interacts with other elements on the screen).
     *
     ...
     * <p>
     * Note that this is the maximum number of framebuffer updates we will request per second. The server does not have to honour our requests. The higher the
     * number of frames per second, the more bandwidth we will consume.
     * <p>
     * Default: 30
     *
    ...
    /**
     * Indicate to the server that the client can draw the mouse pointer locally. The server should not include the mouse pointer in framebuffer updates, and it
     * should send separate notifications when the mouse pointer image changes
     *
     ...
     */
    public void setUseLocalMousePointer(Supplier<Boolean> useLocalMousePointer)
    {
    ...
    {
        if (useLocalMousePointer != null)
        {
            return useLocalMousePointer.get();
        }
        else
        {
            return true;
    ...
    /**
     * Enable or disable the ZLIB video encoding. This encoding is disabled by default because it is very expensive in terms of CPU usage, but it may be useful
     * in situations where you are bandwidth constrained.
     *
