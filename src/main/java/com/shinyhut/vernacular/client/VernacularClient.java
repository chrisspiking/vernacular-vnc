...
public class VernacularClient
{
     ...
     * <p>
     * To indicate a mouse 'click', call this method twice in quick succession, first with pressed = true, then with pressed = false, or use the convenience
     * {@link #click(int)} method.
     * <p>
     * On a conventional mouse, buttons 1, 2, and 3 correspond to the left, middle, and right buttons on the mouse. On a wheel mouse, each step of the wheel
     * upwards is represented by a press and release of button 4, and each step downwards is represented by a press and release of button 5.
     *
     ...
     * <p>
     * This is equivalent to calling {@link #updateMouseButton(int, boolean)} twice in quick succession with pressed = true and pressed = false
     *
     * @param button The mouse button number (1-8)
     */
     ...
     * <p>
     * This is equivalent to calling {@link #updateKey(int, boolean)} twice in quick succession with pressed = true and pressed = false
     *
     * @param keySym The KeySym for this key press or release
     */
     ...
     * <p>
     * Note that this is only guaranteed to work as expected for strings containing only printable ASCII characters - apart from line breaks, which are
     * converted to ENTER key presses, and tabs, which are converted to TAB key presses.
     *
    ...
    public void type(String text)
    {
        ...
        range(0, text.length()).map(text::charAt).map(c ->
                                                      {
                                                          switch (c)
                                                          {
                                                              case '\n':
